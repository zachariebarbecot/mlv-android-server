...
class GoogleMapAPI
{
    ...
    public function addMarkerByCoords($lat, $lng, $title, $html = '', $category = '', $icon = '', $id = '') {
       ...
       $this->contentMarker .= "\t" . 'addMarkerWithLabel(new google.maps.LatLng("' . $lat . '","' . $lng . '"),"' . $title . '","' . $html . '","' . $category . '","' . $icon . '",map' . $this->googleMapId . ',"' . $id . '");' . "\n";
    ...
    public function init() {
        ...
        if ($this->includeJs === TRUE) {
            // Google map JS
            $this->content .= '<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&language=' . $this->lang . '&region=MA">';
            $this->content .= '</script>' . "\n";
            ...
            $this->content .= '<script type="text/javascript" src="http://google-maps-utility-library-v3.googlecode.com/svn/tags/markerwithlabel/1.1.8/src/markerwithlabel.js">';
            $this->content .= '</script>' . "\n";
        }
        ...
        //Js public function to add a marker with Label
        $this->content .= "\t" . 'function addMarkerWithLabel(latlng,title,content,category,icon,currentmap,id) {' . "\n";
        $this->content .= "\t\t" . 'var marker = new MarkerWithLabel({' . "\n";
        $this->content .= "\t\t\t" . 'map:  currentmap,' . "\n";
        $this->content .= "\t\t\t" . 'labelContent:  content,' . "\n";
        $this->content .= "\t\t\t" . 'labelClass:  "labels",' . "\n";
        $this->content .= "\t\t\t" . 'title : title,' . "\n";
        $this->content .= "\t\t\t" . 'icon:  {url:icon,size:new google.maps.Size(' . $this->iconWidth . ', ' . $this->iconHeight . ')';
        if (!empty($this->iconAnchorWidth) || !empty($this->iconAnchorHeight)) {
            $this->content .= ',labelAnchor:new google.maps.Point(' . $this->iconAnchorWidth . ',' . $this->iconAnchorHeight . ')';
        }
        $this->content .= '},' . "\n";
        $this->content .= "\t\t\t" . 'position: latlng' . "\n";
        $this->content .= "\t\t" . '});' . "\n";
